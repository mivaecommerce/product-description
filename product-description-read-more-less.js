...
function readMoreLess($el) {
	...
	var shortDesc = origDesc.split(/\s+/g);  // Strip out all white space and store the words of the description
	...
	if ( (shortDesc.length + 1 ) > cutOff ) {
		...
		shortDesc = shortDesc.slice(0, (cutOff + 1)).join(' ') + '...';
		// Re-populate the passed in element's html with the shortened product description
	...
	$el.on('click', '.read-more', function() {
		// Re-populate the passed in element's html with the original product description
